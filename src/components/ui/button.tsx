...
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-sm border border-transparent bg-clip-padding text-[13px] font-medium whitespace-nowrap transition-all outline-none select-none active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
          "bg-background text-text-tertiary hover:bg-primary hover:text-text-primary",
        outline: "hover:brightness-125 border-border-tertiary bg-bg-quaternary",
        secondary:
          "border border-secondary bg-secondary text-background hover:bg-foreground aria-expanded:bg-secondary aria-expanded:text-background",
        ghost:
          "hover:bg-link-hover hover:text-foreground aria-expanded:bg-link-hover aria-expanded:text-foreground dark:hover:bg-link-hover/50",
        destructive:
      ...
      size: {
        ...
        lg: "h-10 text-[15px] gap-1.5 px-4 has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2",
        icon: "size-8",
        "icon-xs":
